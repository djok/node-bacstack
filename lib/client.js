...
class Client extends EventEmitter {
  ...
  _segmentAckResponse(receiver, negative, server, originalInvokeId, sequencenumber, actualWindowSize) {
    const buffer = this._getBuffer(receiver && receiver.remoteNode);
      ...
      if (!this.emit('unhandledEvent', content)) {
        ...
        if (content.header.expectingReply) {
          ...
          this.client.errorResponse(
            ...
            content.service,
            content.invokeId,
            bacnet.enum.ErrorClass.SERVICES,
            bacnet.enum.ErrorCode.REJECT_UNRECOGNIZED_SERVICE
          );
        }
      }
    }
...
//TODO: update          this._processSegment(address, result.type, result.service, result.invokeId, result.maxSegments, result.maxApdu, true, result.sequencenumber, result.proposedWindowNumber, buffer, offset + result.len, length - result.len);
        }
        break;
      default:
  ...
  _receiveData(buffer, remoteAddress) {
    ...
    switch (result.func) {
      ...
      case baEnum.BvlcResultPurpose.FORWARDED_NPDU:
        ...
        // replies back to.
        header.sender.remoteNode = result.forwardedFrom;
        this._handleNpdu(buffer, result.len, buffer.length - result.len, header);
        break;
   ...
   * @param {object=} options varying behaviour for special circumstances
   * @param {string=} options.forwardTo - If talking to a BBMD, the IP address this message should be relayed to.  (Obtained from the original incoming FORWARDED_NPDU message.)
  ...
  sendBvlc(receiver, buffer) {
    if (receiver && receiver.remoteNode) {
      // Remote node address given, forward to BBMD
      baBvlc.encode(buffer.buffer, baEnum.BvlcResultPurpose.FORWARDED_NPDU, buffer.offset, receiver.remoteNode);
